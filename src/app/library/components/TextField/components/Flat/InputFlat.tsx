...
import {StyleSheet, TextInput, LayoutChangeEvent, View} from 'react-native';
import Animated, {interpolate} from 'react-native-reanimated';
import {useTimingTransition} from '@animated';
import {InputFlatProps} from './InputFlat.props';
import {useTranslation} from 'react-i18next';
import {enhance, onCheckType, scale, verticalScale} from '@common';
...
export const InputFlat = forwardRef<any, InputFlatProps>((props, ref) => {
    ...
    const fontLabel = interpolate(progress, {
        inputRange: [0, 1],
        outputRange: [FontSizeDefault.FONT_18, FontSizeDefault.FONT_16],
    });
        ...
        <Animated.View
            ...
            <Block direction={'row'}>
                <TextInput
                    ...
                    onFocus={_onFocus}
                    onBlur={_onBlur}
                    style={[inputSty, !rightChildren ? {} : {paddingHorizontal: scale(30), right: scale(30)}]}
                    ref={ref}
                    {...rest}
                />
                {rightChildren ? <Block style={{
                    position: 'absolute',
                    right: -2,
                    height: scale(30),
                    width: scale(30),
